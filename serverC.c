...
int main(void){
	...
	if(bind(Usock, (struct sockaddr*)&U_adr, sizeof(U_adr))==-1)
		error_handling("bindUC() error");
        else  
            printf("The ServerC is up and running using UDP on port < 23573 >.\n");
    ...
    while(1){
        ...
        //printf("Message %s\n", message);
        printf("The ServerC receiverd a request from the Main Server.\n");
...
int check(char username[BUF_SIZE]){ // CHECK BALANCE, CHECK WHETHER IN SERVER
    ...
    FILE * fp = fopen("block3.txt", "r");
    if (fp == NULL){
        printf("Block3 not found \n");
        return -1;
    }
